...
class DruidOperator(BaseOperator):
  ...
  def execute(self, context: Context):
    timestamp_spec, schema = read_parquet_schema_for_druid(file = "test.parquet", path = "640a234d-f088-4f23-8796-348711632874")
    payload = {
      "type": "index_parallel",
      "spec": {
        "ioConfig": {
          ...
          "inputSource": {
            ...
            "objects": [
              {
                "bucket": "parquets",
                "path": "640a234d-f088-4f23-8796-348711632874/test.parquet"
              }
            ],
            "properties": {
...
with DAG("test", dag_display_name="test",default_args=default_args,start_date=datetime(2025, 11, 5), schedule_interval="@hourly", description="test", catchup=False, is_paused_upon_creation=False) as dag:
    ...
    hop = HopPipelineOperator(
      ...
      log_level='Basic',
      hop_params={'user_id':'640a234d-f088-4f23-8796-348711632874','minio_ftp':f'{minio_ftp}','dag_id':'test','dag_display_name':'test'}
    )
