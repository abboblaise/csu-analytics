...
def download_pipeline_minio():
    ...
    client.fget_object("pipelines","pipelines-created/640a234d-f088-4f23-8796-348711632874/test.hpl","/hop/config/projects/default/640a234d-f088-4f23-8796-348711632874/test.hpl")
    task_logger.info('Download of pipeline from MinIO completed')

    # Open the file in read-write mode to modify in place
    try:
        with open("/hop/config/projects/default/640a234d-f088-4f23-8796-348711632874/test.hpl", 'r+') as file:
            content = file.read()
...
class DruidOperator(BaseOperator):
  ...
  def execute(self, context: Context):
    timestamp_spec, schema = read_parquet_schema_for_druid(file = "test1.parquet", path = "640a234d-f088-4f23-8796-348711632874")
    payload = {
      "type": "index_parallel",
      "spec": {
        "ioConfig": {
          ...
          "inputSource": {
            ...
            "objects": [
              {
                "bucket": "parquets",
                "path": "640a234d-f088-4f23-8796-348711632874/test1.parquet"
              }
            ],
            "properties": {
...
with DAG("test1", dag_display_name="test1",default_args=default_args,start_date=datetime(2025, 11, 10), schedule_interval="@once", description="TEST", catchup=False, is_paused_upon_creation=False) as dag:
    ...
    hop = HopPipelineOperator(
      ...
      pipeline="640a234d-f088-4f23-8796-348711632874/test.hpl",
      pipe_config='remote hop server',
      project_name='default',
      log_level='Basic',
      hop_params={'user_id':'640a234d-f088-4f23-8796-348711632874','minio_ftp':f'{minio_ftp}','dag_id':'test1','dag_display_name':'test1'}
    )
